...
EventTriggercommand, e.g.`add`Running`chezmoi command`, e.g.`chezmoi add``git-auto-commit`Generating an automatic git commit`git-auto-push`Running an automatic git push`read-source-state`Reading the source state
...
args. commands are executed directly. scripts are executed with configured
...
~/.config/chezmoi/chezmoi.toml[hooks.read-source-state.pre]
command = "echo"
...
~/.config/chezmoi/chezmoi.yamlhooks:
  read-source-state:
    pre:
      command: echo
      args:
