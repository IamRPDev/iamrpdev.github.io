...
See also .chezmoiexternals/ directories.
...
listed in .chezmoiignore), all entries within the file are also
ignored.
...
VariableTypeDefault valueDescription`type`stringnoneExternal type (`file`,`archive`,`archive-file`, or`git-repo`)`decompress`stringnoneDecompression for file`encrypted`bool`false`Whether the external is encrypted`exact`bool`false`Add`exact_`attribute to directories in archive`exclude`[]stringnonePatterns to exclude from archive`executable`bool`false`Add`executable_`attribute to file`private`bool`false`Add`private_`attribute to file`readonly`bool`false`Add`readonly_`attribute to file`format`stringautodetectFormat of archive`path`stringnonePath to file in archive`include`[]stringnonePatterns to include from archive`refreshPeriod`duration`0`Refresh period`stripComponents`int`0`Number of leading directory components to strip from archives`url`stringnoneURL`urls`[]stringnoneExtra URLs to try, in order`checksum.sha256`stringnoneExpected SHA256 checksum of data`checksum.sha384`stringnoneExpected SHA384 checksum of data`checksum.sha512`stringnoneExpected SHA512 checksum of data`checksum.size`intnoneExpected size of data`clone.args`[]stringnoneExtra args to`git clone``filter.command`stringnoneCommand to filter contents`filter.args`[]stringnoneExtra args to command to filter contents`pull.args`[]stringnoneExtra args to`git pull``archive.extractAppleDouble`bool`false`If`true`, AppleDouble files are extracted`targetPath`stringnoneTarget path, overriding the key of the entry
...
archive.extractAppleDouble controls whether AppleDouble files
...
~/.local/share/chezmoi/.chezmoiexternal.toml[".local/bin/zellij"]
    type = "archive-file"
...
~/.local/share/chezmoi/.chezmoiexternal.yaml.local/bin/zellij":
  type: archive-file
  url: {{ gitHubLatestReleaseAssetURL "zellij-org/zellij" "zellij-x86_64-unknown-linux-musl.tar.gz" | quote }}
  executable: true
  path: zellij
...
~/.local/share/chezmoi/.chezmoiexternal.toml[".vim/autoload/plug.vim"]
    type = "file"
...
~/.local/share/chezmoi/.chezmoiexternal.yaml.vim/autoload/plug.vim:
  type: file
  url: https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim
  refreshPeriod: 168h
.oh-my-zsh:
...
~/.local/share/chezmoi/.chezmoiexternal.toml[p10k_fonts]
    type = "archive"
...
~/.local/share/chezmoi/.chezmoiexternal.yamlp10k_fonts:
  type: archive
  url: https://github.com/romkatv/powerlevel10k-media/archive/master.tar.gz
  exact: false
  stripComponents: 1
...
and possible errors can be avoided by using stat:
TOMLYAMLJSON


~/.local/share/chezmoi/.chezmoiexternal.toml{{ if stat (joinPath .chezmoi.homeDir ".ssh" "id_rsa") }}
...
~/.local/share/chezmoi/.chezmoiexternal.yaml{{ if stat (joinPath .chezmoi.homeDir ".ssh" "id_rsa") }}
.path/to/private/repo:
  type: git-repo
  url: git@private.com:org/repo.git
{{ end }}
...
~/.local/share/chezmoi/.chezmoiexternal.json{{ if stat (joinPath .chezmoi.homeDir ".ssh" "id_rsa") }}
{
    ".path/to/private/repo": {
        "type": "git-repo",
        "url": "git@private.com:org/repo.git"
